...
def cluster_model_pipeline(DF, API_KEY, GPT_MODEL):
    ...
    if 'default_cluster' not in st.session_state:
    # Use a try/except block just in case X is malformed here
        try:
            # Pass the processed data to estimate_optimal_clusters
            st.session_state.default_cluster = estimate_optimal_clusters(st.session_state.df_pca)
        except Exception as e:
            # Fallback to a safe default if estimation fails
            st.session_state.default_cluster = 3
            st.warning(f"Could not estimate optimal clusters (Error: {e}). Defaulting to 3.")
        
    st.info(f"AI-based estimation suggests **{st.session_state.default_cluster}** clusters.")
