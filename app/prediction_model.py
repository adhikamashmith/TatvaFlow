...
def prediction_model_pipeline(DF, API_KEY, GPT_MODEL):
    ...
    if st.session_state.target_selected:
        ...
        try:
            st.session_state.df_pca[selected_Y] = st.session_state.df_pca[selected_Y].astype(int)
            st.info(f"Target variable '{selected_Y}' successfully converted to **integer** type for classification.")
        except Exception:
            # If conversion fails (e.g., due to non-numeric data), this warning is useful
            st.warning(f"Could not force conversion of target variable '{selected_Y}' to integer. Ensure it is a valid classification column.")
